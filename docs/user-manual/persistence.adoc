...
journal-lock-monitor-timeout::
How long (in milliseconds) between journal lock health checks. The lock monitor periodically verifies that the journal lock is still held.
+
The default for this parameter is `2000` (i.e. 2 seconds)

journal-lock-monitor-max-retries::
How many consecutive failed lock health checks to tolerate before declaring the lock lost. This is useful for tolerating transient storage outages.
+
The default for this parameter is `0` (i.e. no retries - fail immediately on first check failure)
...
If you are running Linux kernel 2.6 or later and don't already have `libaio` installed you can easily install it using the xref:libaio.adoc#runtime-dependencies[following steps].
